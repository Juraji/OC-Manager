...
class Neo4jDataClassMapper<T : Any>(
    ...
    fun mapFrom(fetchResult: Map<String, Any>, variableName: String): T {
        val root = fetchResult[variableName] ?: throw CanNotMapNullException(baseClass)
        if (root is MapAccessor) return mapFromInternal(root.asMap(), baseClass)
        else throw CanNotMapValueException(root, baseClass)
    }
    ...
    private fun <R : Any> mapFromInternal(mapProjection: Map<String, Any>, resultType: KClass<out R>): R {
        ...
        val parameters = primaryConstructor.parameters.associate { kParameter ->
